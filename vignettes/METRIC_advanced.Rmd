...
Then, we load the weather station data. For that we are going to use the function called `read.WSdata`. This function converts our .csv file into a `waterWeatherStation` object. Then, if we provide a Landsat metadata file (.MTL file) we will be able to calculate the time-specific weather conditions at the time of
satellite flyby.
...
function called `loadImage` to load a Landsat image from `.TIF files` downloaded directly
from [Earth Explorer](http://earthexplorer.usgs.gov/). In this vignette we are
going to use some Landsat 7 as example data wich, by the way comes with **water** package as a demo.
```{r, fig.width = 5}
image.DN <- L7_Talca[[c(1:5,7)]]
B6 <- L7_Talca[[6]]
```

Finally we are going to create The Digital Elevation Model (DEM) for the specific satellite image we are processing. Thus, we needed a image-specifc grid files downloadable from [Earth Explorer](http://earthexplorer.usgs.gov/). Therefore, check wich grid files we are going to need considering the satellite scene location and the AOI polygon, using:

```{r}
checkSRTMgrids(image.DN)
```

You should download all needed grid files, and then you can use the function `prepareSRTMdata(extent = image.DN)` to create the DEM. In this vignette we are going to load the example data provided with **water** package.
